...
#[derive(Debug, Eq, PartialEq, Hash, Clone, Copy, Serialize, Deserialize, Display, EnumIter, TS,)]
...
impl LauncherConfig {
    ...
    fn get_supported_game_config_mut (&mut self,game_name: SupportedGame) -> Result<&mut GameConfig, ConfigError> {
    ...
    pub fn load_config(config_dir: Option<PathBuf>) -> LauncherConfig {
    ...
    pub fn update_setting_value(&mut self, key: &str,val: Value, game_name: Option<SupportedGame>) -> Result<(), ConfigError> {
        if let Some(game_config) = game_name.and_then(|game| self.games.get_mut(&game)) {
            ...
            match key {
                "opengl_requirements_met" => {
                    self.requirements.opengl = val.as_bool().unwrap_or(false)
                }
    ...
    pub fn get_setting_value(&self, key: &str, game_name: Option<SupportedGame>) -> Result<Value, ConfigError> {
        if let Some(game_config) = game_name.and_then(|game| self.games.get(&game)) {
            ...
            match key {
                ...
                "install_directory" => {
                    Ok(self.installation_dir.as_ref().map_or(Value::Null, |v| {
                        Value::String(v.to_string_lossy().into_owned())
                    }))
                }
                "active_version" => Ok(self
                    .active_version
                    .as_ref()
                    .map_or(Value::Null, |v| Value::String(v.clone()))),
                "mod_sources" => Ok(json!(self.mod_sources)),
                "check_for_latest_mod_version" => {
                    Ok(Value::Bool(self.check_for_latest_mod_version))
                }
    ...
    pub fn update_mods_setting_value(&mut self, key: &str, game_name: SupportedGame, source_name: Option<String>, version_name: Option<String>, mod_name: Option<String>, texture_packs: Option<Vec<String>>) -> Result<(), ConfigError> {
    ...
    pub fn cleanup_game_enabled_texture_packs(&mut self, game_name: SupportedGame, cleanup_list: Vec<String>) -> Result<(), ConfigError> {
    ...
    pub fn common_prelude(&self) -> Result<CommonConfigData, CommandError> {
        ...
        let tooling_version =
            Version::parse(version_str).unwrap_or_else(|_| Version::new(0, 1, 35));
